...
import DeviceListCpn, {
  DeviceListCpnRef,
} from "@/components/DeviceListCpn/DeviceListCpn";
...
import CreateTaskCpn from "@/components/CreateInstallUninstallTaskCpn/CreateTaskCpn";

export default function HotDeviceListPage() {
  ...
  const handleEditDevice = useCallback((device: DEVICE_WEB) => {
    toast.info(
      `Chức năng chỉnh sửa cho ${device.deviceName} sẽ được triển khai khi API có sẵn.`
    );
  }, []);
  ...
  const handleCreateTask = useCallback((device: DEVICE_WEB) => {
    setSelectedDevice(device);
  ...
  const confirmDeleteDevice = useCallback(async () => {
    ...
    try {
      console.log(
        `🗑️ Xóa thiết bị: ${selectedDevice.deviceName} (ID: ${selectedDevice.id})`
      );
  ...
  const handleCreateTaskModalClose = useCallback((open: boolean) => {
    setShowCreateTaskModal(open);

    // Remove manual body style changes!
    if (!open) {
      setTimeout(() => {
        setSelectedDevice(null);
      }, 100);
  ...
  const handleTaskCreated = useCallback(async () => {
    // Refresh the device list after task creation
    if (deviceListRef.current) {
      await deviceListRef.current.refetchDevices();
      console.log("✅ Task created and device list refreshed");
    }
  }, []);
  ...
  return (
    <div className="flex flex-col gap-6 p-2">
      ...
      <CreateTaskCpn
        ...
        setOpen={setShowCreateTaskModal} // This is correct
        device={selectedDevice}
        ...
        >
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {selectedDevice && ` "${selectedDevice.deviceName}"`}? Thao tác
