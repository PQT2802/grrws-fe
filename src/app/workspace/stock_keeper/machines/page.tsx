...
export default function StockKeeperMachineListPage() {
    ...
    const handleEditMachine = useCallback((machine: MACHINE_WEB) => {
        toast.info(`Chức năng chỉnh sửa cho ${machine.machineName} sẽ được triển khai khi API có sẵn.`)
    }, [])
    ...
    const confirmDeleteMachine = useCallback(async () => {
        ...
        setIsDeleting(true)
        try {
            console.log(`🗑️ Xóa máy: ${selectedMachine.machineName} (ID: ${selectedMachine.id})`)
            ...
            if (machineListRef.current) {
                ...
                console.log("✅ Máy đã được xóa và danh sách đã được làm mới")
            }
