...
export default function RequestListPage() {
  ...
  const [requestsWithReport, setRequestsWithReport] = useState<
    REQUEST_WITH_REPORT[]
  >([]);
  const [requestsWithoutReport, setRequestsWithoutReport] = useState<
    REQUEST_WITHOUT_REPORT[]
  >([]);
  ...
  // Modal states
  const [selectedRequest, setSelectedRequest] = useState<REQUEST_ITEM | null>(
    null
  );
  ...
  const fetchTabCounts = useCallback(async () => {
    try {
      ...
      if (filterStatus !== "all") {
        filteredRequests = filteredRequests.filter(
          (request) => request.status?.toLowerCase() === filterStatus
        );
      }

      if (filterPriority !== "all") {
        filteredRequests = filteredRequests.filter(
          (request) => request.priority?.toLowerCase() === filterPriority
        );
      }

      // Count requests with and without reports
      const withReportCount = filteredRequests.filter(
        (request) =>
      ...
      const withoutReportCount = filteredRequests.filter(
        (request) =>
          request.reportId === null ||
          request.reportId === undefined ||
          request.reportId === ""
  ...
  const fetchRequests = useCallback(
    async (page: number = 1) => {
      try {
        ...
        setError(null);
        console.log(
          `🔄 Fetching all requests data (page ${page}, size ${pageSize})...`
        );
  ...
  const fetchUserNames = async (requestsData: REQUEST_ITEM[]) => {
    ...
    const uniqueUserIds = [
      ...new Set(requestsData.map((req) => req.createdBy).filter(Boolean)),
    ];
  ...
  const filteredRequestsWithReport = getFilteredRequests(
    requestsWithReport
  ) as REQUEST_WITH_REPORT[];
  const filteredRequestsWithoutReport = getFilteredRequests(
    requestsWithoutReport
  ) as REQUEST_WITHOUT_REPORT[];
