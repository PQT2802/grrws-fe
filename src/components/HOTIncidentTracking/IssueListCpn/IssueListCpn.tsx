...
const IssueListCpn = forwardRef<IssueListCpnRef, IssueListCpnProps>(({
  ...
  const mockIssues: ISSUE_WEB[] = [
    {
      ...
      description: "Machine stopped working unexpectedly during shift, causing production delays",
      severity: "Critical",
      status: "Open",
    ...
    },
    {
      ...
      description: "Defective products found in batch #1234, quality standards not met",
      severity: "High",
      status: "InProgress",
    ...
    },
    {
      ...
      description: "Unsafe working conditions reported, potential hazard for workers",
      severity: "Medium",
      status: "Resolved",
    ...
    },
    {
      ...
      description: "Intermittent network failures affecting multiple departments",
      severity: "High",
      status: "InProgress",
    ...
    },
    {
      ...
      description: "Air conditioning system not working properly, temperature control issues",
      severity: "Medium",
      status: "Open",
    ...
    },
    {
      ...
      description: "Minor chemical spill requiring immediate cleanup and safety assessment",
      severity: "Critical",
      status: "Resolved",
    ...
    },
    {
      ...
      description: "Main elevator out of service, affecting multi-floor access",
      severity: "Medium",
      status: "InProgress",
    ...
    },
    {
      ...
      description: "Fire alarm system triggering false alarms, disrupting operations",
      severity: "High",
      status: "Open",
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case 'Critical': return 'bg-red-500/10 text-red-400 border-red-500/20 dark:bg-red-500/20 dark:text-red-300';
      case 'High': return 'bg-orange-500/10 text-orange-400 border-orange-500/20 dark:bg-orange-500/20 dark:text-orange-300';
      case 'Medium': return 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20 dark:bg-yellow-500/20 dark:text-yellow-300';
      case 'Low': return 'bg-green-500/10 text-green-400 border-green-500/20 dark:bg-green-500/20 dark:text-green-300';
      default: return 'bg-gray-500/10 text-gray-400 border-gray-500/20 dark:bg-gray-500/20 dark:text-gray-300';
    }
  };

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Open': return 'bg-blue-500/10 text-blue-400 border-blue-500/20 dark:bg-blue-500/20 dark:text-blue-300';
      case 'InProgress': return 'bg-purple-500/10 text-purple-400 border-purple-500/20 dark:bg-purple-500/20 dark:text-purple-300';
      case 'Resolved': return 'bg-green-500/10 text-green-400 border-green-500/20 dark:bg-green-500/20 dark:text-green-300';
      case 'Closed': return 'bg-gray-500/10 text-gray-400 border-gray-500/20 dark:bg-gray-500/20 dark:text-gray-300';
      default: return 'bg-gray-500/10 text-gray-400 border-gray-500/20 dark:bg-gray-500/20 dark:text-gray-300';
    }
  };
        ...
        <Table>
          ...
          <TableBody>
            {issues.length === 0 ? (
              ...
              issues.map((issue) => (
                <TableRow key={issue.id} className="hover:bg-muted/30 border-border/50">
                      ...
                      {issue.tags && issue.tags.length > 0 && (
                        <div className="flex items-center gap-1 flex-wrap">
                          ...
                          {issue.tags.slice(0, 3).map((tag) => (
                            <Badge key={tag} variant="outline" className="text-xs px-1.5 py-0.5 h-5">
                              {tag}
                            </Badge>
                          ))}
                          {issue.tags.length > 3 && (
                            <span className="text-xs text-muted-foreground">+{issue.tags.length - 3}</span>
                          )}
                        </div>
                      )}
                    </div>
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-foreground">{issue.reportedBy}</span>
                      </div>
                      {issue.assignedTo && (
                        <div className="flex items-center gap-2">
                          <User className="h-3 w-3 text-blue-400" />
                          <span className="text-sm text-blue-400">→ {issue.assignedTo}</span>
                        </div>
                      )}
                      {!issue.assignedTo && (
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">{formatDate(issue.reportedDate)}</span>
                      </div>
                      {(issue.estimatedResolutionTime || issue.actualResolutionTime) && (
                        <div className="flex items-center gap-2">
                          <Clock className="h-3 w-3 text-muted-foreground" />
                          <span className="text-xs text-muted-foreground">
                            {issue.actualResolutionTime || issue.estimatedResolutionTime}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem onClick={() => onViewIssue(issue)} className="gap-2">
                          <Eye className="h-4 w-4" />
                          View Details
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => onEditIssue(issue)} className="gap-2">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-4 py-3 bg-background/50 dark:bg-muted/20 rounded-lg border">
          <div className="text-sm text-muted-foreground">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, totalItems)} of {totalItems} issues
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
