...
const ErrorListCpn = forwardRef<ErrorListCpnRef, ErrorListCpnProps>(({
  ...
  const mockErrors: ERROR_LOG_WEB[] = [
    {
      ...
      stackTrace: "at DatabaseManager.connect(DatabaseManager.java:45)\nat ServiceLayer.initialize(ServiceLayer.java:23)",
      severity: "Critical",
      source: "Database",
      userId: "usr-001",
    ...
    },
    {
      ...
      stackTrace: "at NetworkClient.connect(NetworkClient.java:82)\nat ExternalAPIService.call(ExternalAPIService.java:34)",
      severity: "Critical",
      source: "Network",
      userId: "usr-004",
    ...
    },
    {
      ...
      stackTrace: "at MemoryIntensiveOperation.process(MemoryIntensiveOperation.java:156)\nat DataProcessor.handleLargeFile(DataProcessor.java:89)",
      severity: "Fatal",
      source: "Application",
    ...
    },
    {
      ...
      stackTrace: "at CacheManager.connect(CacheManager.java:67)\nat CacheService.get(CacheService.java:45)",
      severity: "Critical",
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case 'Fatal': return 'bg-black text-white border-black';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Error Management</h1>
            ...
            {errors.length === 0 ? (
              ...
              errors.map((error) => (
                <TableRow key={error.id} className="hover:bg-gray-50">
                  ...
                  <TableCell>
                    <div>
                      <div className="text-sm font-medium">{error.source}</div>
                      <div className="text-xs text-gray-500">{error.category}</div>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">{error.frequency}x</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-sm">{formatDate(error.occurredAt)}</div>
                        {error.resolvedAt && (
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-700">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, totalItems)} of {totalItems} errors
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
