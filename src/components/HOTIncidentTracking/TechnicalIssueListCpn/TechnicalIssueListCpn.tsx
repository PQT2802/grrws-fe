...
export interface TECHNICAL_ISSUE_WEB {
  ...
  status: 'Reported' | 'InvestigationStarted' | 'FixInProgress' | 'Testing' | 'Resolved';
  deviceId?: string;
  deviceName?: string;
  machineId?: string;
  machineName?: string;
...
const TechnicalIssueListCpn = forwardRef<TechnicalIssueListCpnRef, TechnicalIssueListCpnProps>(({
  onEditTechnicalIssue,
  onDeleteTechnicalIssue,
  onViewTechnicalIssue,
}, ref) => {
  const [technicalIssues, setTechnicalIssues] = useState<TECHNICAL_ISSUE_WEB[]>([]);
  ...
  const mockTechnicalIssues: TECHNICAL_ISSUE_WEB[] = [
    {
      ...
      description: "Controller not responding to commands, possible firmware issue",
      priority: "Urgent",
    ...
    },
    {
      ...
      description: "Hydraulic system losing pressure during operation, affecting production line efficiency",
      priority: "High",
      status: "Reported",
      deviceId: "dev-004",
    ...
    },
    {
      ...
      description: "Switch not properly routing traffic between production networks",
      priority: "Urgent",
      status: "FixInProgress",
      deviceId: "dev-005",
    ...
    },
    {
      ...
      description: "Excessive vibration detected in main drive motor, possible bearing wear",
      priority: "Medium",
      status: "Testing",
      deviceId: "dev-006",
    ...
    },
    {
      ...
      description: "Programmable Logic Controller lost communication with HMI interface",
      priority: "High",
      status: "InvestigationStarted",
      deviceId: "dev-007",
    ...
    },
    {
      ...
      description: "Industrial cooling system not maintaining proper temperature ranges",
      priority: "Medium",
      status: "Resolved",
      deviceId: "dev-008",
    ...
    },
    {
      ...
      description: "Position feedback encoder providing incorrect readings, affecting precision",
      priority: "Urgent",
      status: "FixInProgress",
      deviceId: "dev-009",
    ...
    },
    {
      ...
      description: "Compressed air system unable to maintain consistent pressure levels",
      priority: "Medium",
      status: "Reported",
      deviceId: "dev-010",
    ...
    },
    {
      ...
      description: "Safety system not properly detecting personnel in dangerous zones",
      priority: "Urgent",
      status: "InvestigationStarted",
      deviceId: "dev-011",
    ...
    },
    {
      ...
      description: "VFD running at excessive temperatures, thermal protection activating",
      priority: "High",
      status: "Testing",
      deviceId: "dev-012",
  ...
  const fetchTechnicalIssues = useCallback(async () => {
    try {
      ...
      const filteredIssues = mockTechnicalIssues.filter(issue =>
        ...
        issue.systemComponent.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'InvestigationStarted': return 'bg-purple-100 text-purple-800 border-purple-200';
      case 'FixInProgress': return 'bg-orange-100 text-orange-800 border-orange-200';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Technical Issue Management</h1>
                ...
                <TableRow key={issue.id} className="hover:bg-gray-50">
                  ...
                  <TableCell>
                    {issue.deviceName || issue.machineName ? (
                      <div>
                        ...
                        {issue.machineName && (
                          <div className="text-xs text-gray-500">{issue.machineName}</div>
                        )}
                      </div>
                    ) : (
                      <span className="text-sm text-gray-400">System Level</span>
                    )}
                  </TableCell>
                  <TableCell>
                    {issue.assignedTo ? (
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-sm">{formatDate(issue.reportedDate)}</div>
                        {issue.resolvedDate && (
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-700">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, totalItems)} of {totalItems} technical issues
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
