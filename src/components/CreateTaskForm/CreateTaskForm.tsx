...
const CreateTaskForm = (props: PropType) => {
  ...
  // ✅ Get AuthUser from AuthProvider
  ...
  useEffect(() => {
    if (initValue) {
      ...
      // ✅ Handle dueAt as string instead of Timestamp
      if (initValue?.dueAt) {
        const dueDate =
          typeof initValue.dueAt === "string"
            ? new Date(initValue.dueAt)
            : new Date();
        setDueDate(dueDate);
      }
    }
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // ✅ Use authUser instead of user.uid
    if (
      !authUser?.id ||
      authUser?.id === "" ||
      !workspace?.id ||
    ...
    try {
      if (!isEdit) {
        ...
        const newTask: TASK_TYPE = {
          ...
          dueAt: formatDateForStorage(dueDate), // ✅ Use string format
          createdBy: authUser.id, // ✅ Use authUser.id
        };
      ...
      // ✅ Update permission check to use authUser.id
      if (isEdit && initValue && authUser?.id !== initValue?.createdBy) {
          ...
          <div className="my-6 flex flex-col gap-6">
            ...
            <div className="grid gap-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    {/* ✅ joinUsers still uses USER_TYPE but now with mock data */}
