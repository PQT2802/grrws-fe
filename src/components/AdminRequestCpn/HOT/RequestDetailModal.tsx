...
import { translateTaskStatus, translateTaskPriority, translateTaskType, translateGroupType } from "@/utils/textTypeTask";
...
export default function RequestDetailModal({ request, isOpen, onClose }: RequestDetailModalProps) {
  ...
  const fetchRequestDetails = async () => {
    ...
    try {
      ...
      const [taskGroupsData, technicalIssuesData, errorsData] = await Promise.all([
      ...
      if (taskGroupsData?.data) {
        taskGroupsArray = Array.isArray(taskGroupsData.data) ? taskGroupsData.data : [taskGroupsData.data];
      }
  ...
  const getTaskTypeIcon = (taskType: string) => {
    ...
    switch (taskType.toLowerCase()) {
      ...
      case 'warranty': case 'warrantysubmission': case 'warrantyreturn': return Archive;
      case 'replacement': return RefreshCw;
      case 'installation': case 'uninstallation': return Settings;
  ...
  const getStatusIcon = (status: string) => {
    ...
    switch (status.toLowerCase()) {
      ...
      case 'inprogress': case 'in-progress': return RefreshCw;
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      <div className="flex flex-col space-y-1">
                        <span className="text-sm font-medium text-muted-foreground">Tiêu đề yêu cầu</span>
                        <span className="text-sm">{request.requestTitle || 'N/A'}</span>
                      </div>
                      
                      <div className="flex flex-col space-y-1">
                        <span className="text-sm font-medium text-muted-foreground">Mô tả</span>
                        <span className="text-sm">{request.description || 'Không có mô tả'}</span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <span className="text-sm font-medium text-muted-foreground">Trạng thái</span>
                          <Badge className={`${getStatusColor(request.status)} w-fit px-3 py-1`}>
                            {safeTranslateTaskStatus(request.status)}
                                      ...
                                      <div className="flex flex-col space-y-2">
                                        ...
                                        <Badge className={`${getPriorityColor(request.priority)} w-fit px-3 py-1`}>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-sm">{createdByName || 'Loading...'}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-sm">{request.areaName || 'N/A'} - {request.zoneName || 'N/A'} (Vị trí {request.positionIndex || 'N/A'})</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-muted-foreground">Thiết bị</span>
                          <span className="text-sm">{request.deviceName || 'N/A'} ({request.deviceCode || 'N/A'})</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    {technicalIssues.length > 0 && (
                      <div>
                        <h4 className="font-medium mb-3 text-blue-600">Triệu chứng kỹ thuật ({technicalIssues.length})</h4>
                        <div className="space-y-3">
                          {technicalIssues.map((issue, index) => (
                            <div key={index} className="relative p-4 bg-blue-50 border-l-4 border-blue-500 rounded-lg dark:bg-blue-950/20">
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="font-medium text-blue-800 dark:text-blue-200">
                                      {issue.name || issue.displayName || 'Unnamed Technical Issue'}
                                    </span>
                                    {issue.isCommon && (
                    ...
                    {errors.length > 0 && (
                      <div>
                        <h4 className="font-medium mb-3 text-orange-600">Lỗi kỹ thuật ({errors.length})</h4>
                        <div className="space-y-3">
                          {errors.map((error, index) => (
                            <div key={index} className="relative p-4 bg-orange-50 border-l-4 border-orange-500 rounded-lg dark:bg-orange-950/20">
                              <div className="flex items-start justify-between">
                                <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="font-medium text-orange-800 dark:text-orange-200">
                                      {error.errorName || error.name || 'Unknown Error'}
                                    </span>
                                    {error.isCommon && (
                                      <Badge variant="outline" className="bg-yellow-100 text-yellow-800 border-yellow-300">
                                        Phổ biến
                ...
                <CardContent>
                  {taskGroups.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p>Không có nhóm công việc nào được tạo cho yêu cầu này</p>
                    </div>
                  ) : (
                    <div className="space-y-6">
                                          ...
                                          <TableCell>
                                            <div className="space-y-1">
                                              <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-medium">{task.taskName || 'Unnamed Task'}</span>
                                              </div>
                                              <div className="text-sm text-muted-foreground line-clamp-2 ml-7">
                                                {task.taskDescription || task.description || 'No description'}
                                              </div>
                                            </div>
                                          </TableCell>
                                          <TableCell>
                                            <div className="flex items-center gap-2">
                                              ...
                                              <span className="text-sm">{safeTranslateTaskType(task.taskType)}</span>
                                            </div>
                                          </TableCell>
                                          <TableCell>
                                            <Badge className={`${getStatusColor(task.status)} flex items-center gap-1 w-fit`}>
                                              <StatusIcon className="h-3 w-3" />
                                              {safeTranslateTaskStatus(task.status)}
                                            </Badge>
                                          </TableCell>
                                          <TableCell>
                                            <Badge className={getPriorityColor(task.priority)}>
                                              {safeTranslateTaskPriority(task.priority)}
                                            </Badge>
                                          </TableCell>
                                          <TableCell>
                                            ...
                                            <div className="space-y-1">
                                              {task.startTime && (
                                                <div className="text-sm">
                                                  Bắt đầu: {formatDate(task.startTime)}
                                                </div>
                                              )}
                                              {task.expectedTime && (
                                                <div className="text-xs text-orange-400">
                                                  Dự kiến: {formatDate(task.expectedTime)}
                                                </div>
                                              )}
                                              {task.endTime && (
                                                <div className="text-xs text-green-400">
                                                  Hoàn thành: {formatDate(task.endTime)}
                                                </div>
                                              )}
                                            </div>
                                          </TableCell>
                        ...
                        <div key={index} className="p-3 bg-muted/30 rounded-lg">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="font-medium text-sm">{issue.displayName || 'Unnamed Issue'}</span>
                            </div>
                            {issue.imageUrls && issue.imageUrls.length > 0 && (
                              <div className="flex items-center gap-2">
                                ...
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => handleImageClick(issue.imageUrls[0])}
