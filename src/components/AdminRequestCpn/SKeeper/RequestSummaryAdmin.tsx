...
export default function RequestSummaryAdmin() {
  const [summaryData, setSummaryData] = useState<RequestSummaryData | null>(
    null
  );
  ...
  const [selectedRequest, setSelectedRequest] =
    useState<UNIFIED_SKEEPER_REQUEST | null>(null);
  ...
  }, [fetchAllRequests, requestsPagination.pageSize]);
  ...
  const fetchSummaryData = async () => {
    try {
      ...
      const machineActionStats = {
        ...
        stockIn: machineActionData.filter(
          (req: any) => req.actionType?.toLowerCase() === "stockin"
        ).length,
        stockOut: machineActionData.filter(
          (req: any) => req.actionType?.toLowerCase() === "stockout"
        ).length,
        installation: machineActionData.filter(
          (req: any) => req.actionType?.toLowerCase() === "installation"
        ).length,
        warrantySubmission: machineActionData.filter(
          (req: any) => req.actionType?.toLowerCase() === "warrantysubmission"
        ).length,
        sparePartRequest: machineActionData.filter(
          (req: any) => req.actionType?.toLowerCase() === "sparepartrequest"
        ).length,
        pending: machineActionData.filter(
          (req: any) => req.status?.toLowerCase() === "pending"
        ).length,
        completed: machineActionData.filter(
          (req: any) => req.status?.toLowerCase() === "completed"
        ).length,
        inprogess: machineActionData.filter(
          (req: any) => req.status?.toLowerCase() === "inprogress"
        ).length,
      };
  ...
  const availableActionTypes = useMemo(() => {
    return [
      ...new Set(allRequests.map((req) => req.actionType).filter(Boolean)),
    ];
  }, [allRequests]);
  ...
  const generatePageNumbers = () => {
    ...
    if (requestsPagination.totalPages <= maxVisiblePages) {
      ...
      if (requestsPagination.currentPage <= 3) {
        ...
        requestsPagination.totalPages - 2
      ) {
        pages.push(1);
        pages.push("ellipsis");
        for (
          let i = requestsPagination.totalPages - 3;
          i <= requestsPagination.totalPages;
          i++
        ) {
          pages.push(i);
        }
        ...
        pages.push("ellipsis");
        for (
          let i = requestsPagination.currentPage - 1;
          i <= requestsPagination.currentPage + 1;
          i++
        ) {
          pages.push(i);
        }
  ...
  useEffect(() => {
    if (!isLoading) {
      // Only refetch if initial load is complete
      fetchAllRequests(1, requestsPagination.pageSize);
    }
  }, [
    statusFilter,
  ...
  const handleViewRequest = async (request: UNIFIED_SKEEPER_REQUEST) => {
    try {
      ...
      const detailResponse = await apiClient.machineActionConfirmation.getById(
        request.id
      );
        ...
        <Card className="border border-slate-200 dark:border-slate-700">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {summaryData.machineActions.total}
            </div>
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="bg-gray-100 dark:bg-gray-900 text-gray-600 dark:text-gray-200">
                    <th className="px-4 py-3 text-left font-medium w-[350px]">
                      Mã xác nhận
                    </th>
                    ...
                    <th className="px-4 py-3 text-center font-medium w-[90px]">
                      Thao tác
                    </th>
                  </tr>
                </thead>
