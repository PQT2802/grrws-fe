...
import { Eye, Search, Filter, Settings, Calendar, User, Clock, CheckCircle, XCircle, Package, Archive, Cog, RefreshCw } from 'lucide-react';
import { 
  Pagination, 
  PaginationContent, 
  PaginationItem, 
  ...
  const fetchRequests = async (page: number = 1, pageSize: number = 10) => {
    try {
      ...
      setPagination({
        currentPage,
        pageSize,
        totalItems: machineRequests.length, // Use filtered count
