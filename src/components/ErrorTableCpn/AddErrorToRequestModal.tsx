...
import { X, Check, Loader2, Search, AlertCircle, Plus } from "lucide-react";
...
const AddErrorToRequestModal = ({
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        // Filter out errors that are already selected
  ...
  }, [searchQuery, selectedErrors, open]);
  ...
  const handleSelectError = (error: ErrorSuggestion) => {
    // Add the error to selected list
    setSelectedErrors((prev) => [...prev, error]);

    // Filter out the selected error from suggestions immediately
    setSuggestions((prev) =>
    ...
    // IMPORTANT: Add these lines to explicitly keep the dropdown open
    setCommandOpen(true);

    // Refresh suggestions with current query after a brief delay
    setTimeout(() => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      // Keep dropdown open if click is inside search input or dropdown
      if (
