...
interface MechanicStatusPieChartProps {
  data: ChartDataItem[];
  total: number;
}

export default function MechanicStatusPieChart({ data, total }: MechanicStatusPieChartProps) {
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      const data = payload[0].payload;
      const vietnameseName = data.name === "Available" ? "Có sẵn" : "Đang làm việc";
      return (
        <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-lg border">
          <p className="font-semibold">{vietnameseName}</p>
        ...
        <PieChart>
          ...
          >
            {data.map((entry, index) => (
              <Cell key={`cell-mechanics-${index}`} fill={entry.color} />
            ))}
          </Pie>
            ...
            <div key={index} className="flex items-center justify-between p-2 rounded-lg border bg-muted/20">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className={`font-medium text-sm ${
                  item.name === "Available" ? "text-green-600" : "text-red-600"
                } truncate`}>
                  {item.name === "Available" ? "Có sẵn" : "Đang làm việc"}
                </span>
              </div>
